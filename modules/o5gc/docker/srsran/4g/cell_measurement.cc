                  ...
                  if (! sib_decoded) {
                    for (uint32_t tb = 0; tb < SRSRAN_MAX_CODEWORDS; tb++) {
                      if (pdsch_cfg.grant.tb[tb].enabled) {
                        asn1::cbit_ref bref{data[tb], (uint32_t)(1 + (pdsch_cfg.grant.tb[tb].tbs - 1) / 8)};
                        ...
                        if (dlsch_msg.msg.c1().type() == asn1::rrc::bcch_dl_sch_msg_type_c::c1_c_::types::sib_type1) {
                          ...
                          //srsran_vec_fprint_byte(stdout, data[tb], 1 + (pdsch_cfg.grant.tb[tb].tbs - 1) / 8);
                          //asn1::json_writer json_writer;
                          //dlsch_msg.msg.c1().sib_type1().to_json(json_writer);
                          //printf("%s", json_writer.to_string().c_str());
                          asn1::rrc::sib_type1_s & sib1 (dlsch_msg.msg.c1().sib_type1());
                          printf("Decoded SIB1.");
                          for (const auto& e : sib1.cell_access_related_info.plmn_id_list) {
                            if (e.plmn_id.mcc_present) {
                              printf("  MCC: ");
                              for (const auto& d : e.plmn_id.mcc) { printf("%d", d); }
                            }
                            printf("  MNC: ");
                            for (const auto& d : e.plmn_id.mnc) { printf("%d", d); }
                          }
                          printf("  TAC: %lu", sib1.cell_access_related_info.tac.to_number());
                          printf("  CellId: %lu", sib1.cell_access_related_info.cell_id.to_number());
                          printf("\n");
                          sib_decoded = 1;
                        }
                      }
...
void* plot_thread_run(void* arg)
{
  ...
  plot_scatter_setTitle(&pscatequal, (char *)"PDSCH - Equalized Symbols");
  ...
  if (enable_mbsfn_plot) {
    ...
    plot_scatter_setTitle(&pscatequal_pmch, (char *)"PMCH - Equalized Symbols");
  ...
  if (!prog_args.disable_plots_except_constellation) {
    ...
    plot_real_setTitle(&p_sync, (char *)"PSS Cross-Corr abs value");
    plot_real_setYAxisScale(&p_sync, 0, 1);

    plot_scatter_init(&pscatequal_pdcch);
    plot_scatter_setTitle(&pscatequal_pdcch, (char *)"PDCCH - Equalized Symbols");
  ...
  while (1) {
    ...
    if (!prog_args.disable_plots_except_constellation) {
      for (i = 0; i < nof_re; i++) {
        tmp_plot[i] = srsran_convert_amplitude_to_dB(std::abs(std::complex<float>(ue_dl.sf_symbols[0][i])));
        if (isinf(tmp_plot[i])) {
          tmp_plot[i] = -80;
        }
      }
      ...
      for (i = 0; i < 12 * ue_dl.cell.nof_prb; i++) {
        tmp_plot2[g + i] = srsran_convert_amplitude_to_dB(std::abs(std::complex<float>(ue_dl.chest_res.ce[0][0][i])));
